...
t('UI: 우측 필수입력 패널·Alt+I 전체화면 질답·drag 네이밍 제공', () => {
  ...
  assert.ok(SHARED_WIZARD.includes('isCliWizardRequired') && SHARED_WIZARD.includes('필수 입력 모드')
    && SHARED_WIZARD.includes('question.essential') && SHARED_WIZARD.includes('onKeyDown={onKeyDown}')
    && SHARED_WIZARD.includes('event.defaultPrevented') && SHARED_WIZARD.includes('event.nativeEvent.isComposing'))
