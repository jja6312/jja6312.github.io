...
- [~] `P2.5-02` Pure engine — 기반 완료(green): `blueprintTypes.d.mts`(공유 타입), `jsonCanonical.mjs`+`blueprintCanonical.ts`(RFC 8785+SHA-256, 브라우저·Node 공용), `blueprintGraph.mjs`(topo/reverse/ancestors/cycle), `shellQuote.mjs`(POSIX 인용). `.mjs`+`.d.mts` 아키텍처 확정(Node 직접 테스트 + TS 앱 import, `tsc -b` 통과). `scripts/test-blueprint-engine.mjs` 15건 통과. **남음: Naming(derive 10키)·Plan(discovery→state)·Render(Discover/Apply/Resume/Verify/Rollback bash)·Manifest(parse/merge/digest) — cloud create/delete bash 생성부라 리뷰 기반 증분 구현 필요**
